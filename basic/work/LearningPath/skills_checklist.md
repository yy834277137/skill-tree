...
- 今日模块：LeetCode 刷题（链表 + 堆 / 第 k 小）
- 今日题目（已完成）：LC21 86 23 141 142 876 19 160 264 378 373 82 2 445
- 特别标记（需要优先队列 + lambda 表达式）：
  - LC23 合并 K 个升序链表
  - LC378 有序矩阵中第 K 小的元素
  - LC373 查找和最小的 K 对数字
- 我自己的理解：今天题单的主线是「链表哑结点 + 快慢指针」与「最小值枚举的堆（优先队列）模型」。需要把 `priority_queue` 的自定义比较（lambda/仿函数）写熟，避免面试时卡在语法层。
- 实践动作：在 `leetcode-guide.html` 里对照题面示例，把 23/378/373 三题各写一版“最小堆 + 结构体/tuple + lambda 比较”的模板实现（不要求最优，但要求一次过编译）。
- 今天状态：[x]
...
2、数组双指针技巧（12 题）
- 去重/原地移除：LC26、LC27、LC80
- 有序数组双指针：LC83、LC88、LC167、LC977
- 交换/原地重排：LC75
- 原地移动：LC283
- 反转字符串：LC344
- 回文判断：LC125
...
- 节奏建议：
  - 先做「递归链表」两题（206、92）热身，把递归/迭代两套写法都能顺手写出来，再上 25（边界多）。
  - 数组双指针按“模板复用”顺序刷：26/27/283 → 83/88/167/977 → 75 → 125 → 80 → 5。
- 今日产出目标：
  - 每题一句话：输入/输出、双指针含义（slow/fast 或 left/right）、循环不变量。
  - 复用两个模板：链表翻转（含区间反转）与数组原地覆盖（write 指针）。
- 今天状态：[ ] / [~] / [x] / [!]
