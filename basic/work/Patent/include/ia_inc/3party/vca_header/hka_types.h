...
* 作    者：武晓阳
* 日    期：2021年11月24日
* 备    注：增加HKA_ADDR数据类型，表示跨平台内存地址值；
* 
* 当前版本：1.2.6
* 作    者：武晓阳
* 日    期：2020年5月9日
* 备    注：修改ASIC情况下HKA_MEM_TAB_TYPE缺失问题；
* 
* 当前版本：1.2.5
* 作    者：武晓阳
* 日    期：2019年3月1日
...
typedef double              HKA_F64;
    ...
    typedef HKA_U64      HKA_ADDR;
#else
    typedef HKA_U32      HKA_SZT;
    typedef HKA_U32      HKA_ADDR;
#endif
...
typedef enum _HKA_MEM_TAB_TYPE
{
    HKA_MEM_TAB_DDR_PERSIST = 0,     // 不可复用内存
    HKA_MEM_TAB_DDR_SCRATCH,         // DDR缓存中可复用内存，能在多路切换时有条件复用
    HKA_MEM_TAB_RAM_SCRATCH,         // 模拟ASIC中RAM缓存
    HKA_MEM_TAB_NUM                  // 缓存表总个数
} HKA_MEM_TAB_TYPE;
