...
#define PES_PRIVT_BASE          STREAM_TYPE_HIK_PRIVT

#define MAX_PES_PACKET_LEN      65496
#define MIN_PES_HEAD_STUFF_LEN	2

#define PS_PRIVATE_HEAD_LEN     12  ///< hik私有数据包头长度
...
typedef struct _HIK_PROGRAM_STREAM_INFO_
{
    unsigned int	video_stream_id;
	unsigned int	audio_stream_id;
    unsigned int    privt_stream_id;
	unsigned int	PSM_version;

    unsigned int	stream_mode;         /* 输入流模式*/
	unsigned int	max_byte_rate;	    /* 码率，以byte为单位*/
    unsigned int    max_pes_len;        /* 最大pes长度*/
    unsigned int    encrypt_type;       /* 加密模式 */
    unsigned int    video_clip;
	unsigned int    dscpt_sets;
	unsigned int    bframe_audio_set_psh;
    ...
    //描述子内存空间
    unsigned char   device_dsc[HIK_DEVICE_DESCRIPTOR_LEN];
    unsigned char   video_dsc[HIK_VIDEO_DESCRIPTOR_LEN];
    unsigned char   audio_dsc[HIK_AUDIO_DESCRIPTOR_LEN];
    unsigned char   video_clip_dsc[HIK_VIDEO_CLIP_DESCRIPTOR_LEN];
    unsigned char   timing_hrd_dsc[HIK_TIMING_HRD_DESCRIPTOR_LEN];
	unsigned char   privte_header[PS_PRIVATE_HEAD_LEN];
...
* 返回值：crc校验码
******************************************************************************/
unsigned int PSMUX_mpeg2_crc(const unsigned char *buffer, unsigned int length)
{
    ...
    if(!PSMUX_CTX[256])
	{
        ...
        while(buffer<end-3)
		{
            crc ^= (*(unsigned int*)buffer);
				buffer+=4;
            crc =  PSMUX_CTX[3*256 + ( crc     &0xFF)]
                  ^PSMUX_CTX[2*256 + ((crc>>8 )&0xFF)]
                  ^PSMUX_CTX[1*256 + ((crc>>16)&0xFF)]
                  ^PSMUX_CTX[0*256 + ((crc>>24)     )];
        }
		#endif
	}
	else
	{
	    ///printf("PSMUX_CTX[256] is %d\n",PSMUX_CTX[256]);
	}
...
//    }
	...
	buffer[6] =   (1<<7)//  2 - check bits '10'
                | ((prc->encrypt_round & 0x03) << 4)//  2 - PES_scrambling_control(0)
...
* 返回值：PS 头数据的长度
******************************************************************************/
int PSMUX_fill_PSH(unsigned char *buffer, PSMUX_PROCESS_PARAM *prc, HIK_PS_INFO* prg)
{
	unsigned int mux_rate     = 0;
	//unsigned int stuffing_len = 0;
	...
	buffer[pos++]	= 0x40					            //2bits	MPEG2标志 "01"
				    | ((prc->sys_clk_ref>>26) & 0x38) 	//3bits	system_clock_reference_base [32..30]
				    | 0x04					            //1bits	marker_bit
				    | ((prc->sys_clk_ref>>27) & 0x03);	//2bits	system_clock_reference_base [29..28]
	buffer[pos++]	= (prc->sys_clk_ref>>19) & 0xff;    //8bits	system_clock_reference_base [27..20]
	buffer[pos++]	= ((prc->sys_clk_ref>>11) & 0xf8)	//5bits	system_clock_reference_base [19..15]
				    | 0x04					            //1bits	marker_bit
				    | ((prc->sys_clk_ref>>12) & 0x03);	//2bits	system_clock_reference_base [14..13]
	buffer[pos++]	= (prc->sys_clk_ref>>4) & 0xff;		//8bits	system_clock_reference_base [12..5]
	buffer[pos++]	= ((prc->sys_clk_ref<<4) & 0xf0)	//5bits	system_clock_reference_base [4..0]
				    | 0x04					//1bits	marker_bit
...
* 返回值：PSM 数据的长度
******************************************************************************/
int PSMUX_fill_PSM(unsigned char * buffer,  HIK_PS_INFO *prg, PSMUX_PROCESS_PARAM *prc)
{
	...
	if (prg->dscpt_sets & INCLUDE_BASIC_DESCRIPTOR)
	{
		tmp_len += HIK_BASIC_DESCRIPTOR_LEN;
	}
	if (prg->dscpt_sets & INCLUDE_DEVICE_DESCRIPTOR)
	{
		tmp_len += HIK_DEVICE_DESCRIPTOR_LEN;
	}

    if (prg->dscpt_sets & INCLUDE_ENCRYPT_DESCRIPTOR)
    {
        if (prc->encrypt_round != HIK_PSPACK_SECRET_ROUND00
         && prc->encrypt_type != HIK_PSPACK_SECRET_TYPE_ZERO
         && prc->encrypt_arith != HIK_PSPACK_SECRET_ARITH_NONE
         && prc->key_len != HIK_PSPACK_SECRET_KEYLEN000)
        {
             tmp_len += HIK_ENCRYPT_DESCRIPTOR_LEN;
        }
    }
	...
	if (prg->dscpt_sets & INCLUDE_BASIC_DESCRIPTOR)
	{
		pos +=  HKDSC_fill_basic_descriptor(&buffer[pos], &prc->global_time, prg->encrypt_type, prc->company_mark, prc->camera_mark);
	}

	if (prg->dscpt_sets & INCLUDE_DEVICE_DESCRIPTOR)
	{
		memcpy(&buffer[pos], prg->device_dsc, HIK_DEVICE_DESCRIPTOR_LEN);
		pos +=  HIK_DEVICE_DESCRIPTOR_LEN;
	}
    ...
    if (prg->dscpt_sets & INCLUDE_ENCRYPT_DESCRIPTOR)
    {
        if (prc->encrypt_round != HIK_PSPACK_SECRET_ROUND00
         ...
         && prc->key_len != HIK_PSPACK_SECRET_KEYLEN000)
        {
             pos += HKDSC_fill_encrypt_descriptor(&buffer[pos], prc->encrypt_round, prc->encrypt_type, prc->encrypt_arith, prc->key_len);
        }
	...
	if (prg->stream_mode & INCLUDE_VIDEO_STREAM)
	{
		...
		if (prg->dscpt_sets & INCLUDE_VIDEO_DESCRIPTOR)
		{
			tmp_len += HIK_VIDEO_DESCRIPTOR_LEN;
			memcpy(&buffer[pos], prg->video_dsc, HIK_VIDEO_DESCRIPTOR_LEN);
			pos +=  HIK_VIDEO_DESCRIPTOR_LEN;
		}
		if (prg->video_clip || (prg->dscpt_sets & INCLUDE_VIDEO_CLIP_DESCRIPTOR))
		{
			tmp_len += HIK_VIDEO_CLIP_DESCRIPTOR_LEN;
			memcpy(&buffer[pos], prg->video_clip_dsc, HIK_VIDEO_CLIP_DESCRIPTOR_LEN);
			pos         +=  HIK_VIDEO_CLIP_DESCRIPTOR_LEN;
		}
		if (prg->dscpt_sets & INCLUDE_TIMING_HRD_DESCRIPTOR)
		{
			tmp_len += HIK_TIMING_HRD_DESCRIPTOR_LEN;
			memcpy(&buffer[pos], prg->timing_hrd_dsc, HIK_TIMING_HRD_DESCRIPTOR_LEN);
			pos += HIK_TIMING_HRD_DESCRIPTOR_LEN;
		}
	...
	{
		buffer[pos++] = prg->audio_stream_type;
		buffer[pos++] = prg->audio_stream_id;
		if (prg->dscpt_sets & INCLUDE_AUDIO_DESCRIPTOR)
		{
			buffer[pos++] = (unsigned char)(HIK_AUDIO_DESCRIPTOR_LEN >> 8);
			buffer[pos++] = (unsigned char)(HIK_AUDIO_DESCRIPTOR_LEN);
			memcpy(&buffer[pos], prg->audio_dsc, HIK_AUDIO_DESCRIPTOR_LEN);
			pos         +=  HIK_AUDIO_DESCRIPTOR_LEN;
		}
		else
		{
			buffer[pos++] = 0;
    ...
    if (prg->stream_mode & INCLUDE_PRIVT_STREAM)
    {
		buffer[pos++] = prg->privt_stream_type;
		buffer[pos++] = prg->privt_stream_id;
		buffer[pos++] = 0;
		buffer[pos++] = 0;
    }
...
int PSMUX_PrivHead_Modify(HIK_PS_INFO *prg, PES_PACK_INFO *pes)
{
	if (pes->add_pts && prg->max_pes_len)
	{
		prg->privte_header[2] = (((pes->payload_len + 8 - PS_PRIVATE_HEAD_LEN) / 4) >> 8);
		prg->privte_header[3] = ((pes->payload_len + 8 -PS_PRIVATE_HEAD_LEN) / 4);
		prg->privte_header[7] = (pes->total_data_len / prg->max_pes_len) + 1;
		prg->privte_header[8] = 0;
	}
	else
	{
		// +8 私有头除type、len字段还有8字节
		prg->privte_header[2] = (((pes->payload_len + 8) / 4) >> 8);
		prg->privte_header[3] = ((pes->payload_len + 8) / 4);
		prg->privte_header[8] = prg->privte_header[8]+1;
	}
...
int PSMUX_GetMemSize(PSMUX_PARAM *param)
{
	if (param == NULL)
    {
		return HIK_PSMUX_LIB_E_PARA_NULL;
    }
	param->buffer_size = sizeof(HIK_PS_INFO);
	return HIK_PSMUX_LIB_S_OK;
}
...
int PSMUX_Create(PSMUX_PARAM *param, void **handle)
{
    ...
    prg = (HIK_PS_INFO *)param->buffer;

    PSMUX_ResetStreamInfo((void *)prg, &param->info);
	prg->PSM_version		= 0;

	prg->video_stream_id = PES_VIDEO_BASE;
	prg->audio_stream_id = PES_AUDIO_BASE;
    prg->privt_stream_id = PES_PRIVT_BASE;
...
{
	HIK_PS_INFO *prg = (HIK_PS_INFO *)handle;
    int frame_rate;
    ...
    prg->audio_stream_type     = info->audio_stream_type;  /* 输入音频流类型 */
	prg->privt_stream_type     = info->privt_stream_type;	/* 输入私有流类型 */
	prg->dscpt_sets            = info->dscrpt_sets;
	prg->bframe_audio_set_psh  = info->bframe_audio_set_psh;
	prg->set_frame_end_flg     = info->set_frame_end_flg;

	if (prg->dscpt_sets & INCLUDE_DEVICE_DESCRIPTOR)
	{
		HKDSC_fill_device_descriptor(prg->device_dsc, info->stream_info.dev_chan_id);
	}
    ...
    if (info->stream_mode & INCLUDE_VIDEO_STREAM)
    {
        if (prg->dscpt_sets & INCLUDE_VIDEO_DESCRIPTOR)
		{
			HKDSC_fill_video_descriptor(prg->video_dsc, &info->stream_info.video_info);
		}
        if (prg->video_clip || (prg->dscpt_sets & INCLUDE_VIDEO_CLIP_DESCRIPTOR))
        {
            HKDSC_fill_video_clip_descriptor(prg->video_clip_dsc, &info->stream_info.video_info);
        }
		if (prg->dscpt_sets & INCLUDE_TIMING_HRD_DESCRIPTOR)
		{
			if (info->stream_info.video_info.time_info != 0)
			{
				frame_rate = 90000 / info->stream_info.video_info.time_info;
			}
			else
			{
				frame_rate = 25;
			}
			HKDSC_fill_timing_hrd_descriptor(prg->timing_hrd_dsc, frame_rate, info->stream_info.video_info.width_orig,
		    info->stream_info.video_info.height_orig);
		}
    }
    if (info->stream_mode & INCLUDE_AUDIO_STREAM)
    {
        if (prg->dscpt_sets & INCLUDE_AUDIO_DESCRIPTOR)
		{
			HKDSC_fill_audio_descriptor(prg->audio_dsc, &info->stream_info.audio_info);
		}
    ...
    prg->max_pes_len = info->max_packet_len - (info->max_packet_len & 3) - PS_PRIVATE_HEAD_LEN;
...
int PSMUX_Process(void *handle, PSMUX_PROCESS_PARAM *param)
{
	...
	else 
	{
        ...
        else if(param->frame_type == FRAME_TYPE_PRIVT_FRAME)
        {
			...
			prg->video_stream_type = STREAM_TYPE_PRIVT_DATA;
        }            
		else 
		{
			if (param->frame_type == AUDIO_G711_U)
			{
			    prg->audio_stream_type = STREAM_TYPE_AUDIO_G711U;
			}
			else if (param->frame_type == AUDIO_G711_A)
			{
			    prg->audio_stream_type = STREAM_TYPE_AUDIO_G711A;
			}
			else
			{
			    SAL_DEBUG("encodType error %d\n",param->frame_type );
				prg->audio_stream_type = STREAM_TYPE_AUDIO_AAC;
			}
			

			param->frame_type = FRAME_TYPE_AUDIO_FRAME;
		}
    ...
    case FRAME_TYPE_AUDIO_FRAME:
		pes->stream_id = prg->audio_stream_id;
        break;
    case FRAME_TYPE_PRIVT_FRAME:
        pes->stream_id = prg->privt_stream_id;
		memcpy(prg->privte_header, param->unit_in_buf, PS_PRIVATE_HEAD_LEN);
        break;
    default:
        return HIK_PSMUX_LIB_E_STREAM_TYPE;
	}
    ...
    if (param->is_first_unit && param->is_unit_start)
    {
         ...
         || (pes->stream_id == prg->video_stream_id && param->frame_type != FRAME_TYPE_VIDEO_BFRAME))
        {
            ...
            {
		        return HIK_PSMUX_LIB_E_MEM_OVER;
            }
        }

		if (prg->bframe_audio_set_psh && param->frame_type != FRAME_TYPE_VIDEO_IFRAME
			&& param->frame_type != FRAME_TYPE_VIDEO_PFRAME
			&& param->is_first_unit && param->is_unit_start)
		{
			pos += PSMUX_fill_PSH(buffer, param, prg);
//			pos += PSMUX_fill_PSH(buffer, prg->max_byte_rate, param->sys_clk_ref, param->frame_num, prg->set_frame_end_flg);
			if (pos >= param->out_buf_size)
			{
				return HIK_PSMUX_LIB_E_MEM_OVER;
			}
...
/*    pes->add_user_data   = (param->add_userdata && param->is_first_unit); */
    ...
    do
    {
		if (FRAME_TYPE_PRIVT_FRAME == param->frame_type
			&& 0x01 == param->unit_in_buf[0]
		    && 0x01 == param->unit_in_buf[1]) // 只对温度信息有效
		{
			mod_pos = pos;
			pos += PSMUX_create_pes_header(&buffer[pos], pes, param, prg);

			pes_mod_len = pes->payload_len & 3;
			if (pes_mod_len)
			{
				pes->payload_len -= pes_mod_len;
			}

			PSMUX_PrivHead_Modify(prg, pes);

			if (!pes->add_pts)
			{
				//pes->payload_len += PS_PRIVATE_HEAD_LEN;
				memcpy(&buffer[pos], prg->privte_header, PS_PRIVATE_HEAD_LEN);
			...
			{
				//覆盖原先的12字节头
				memcpy(param->unit_in_buf, prg->privte_header, PS_PRIVATE_HEAD_LEN);
			}
		}
		else
		{
...
int MUX_Ps_update_vdp(void *handle, HIKPSMX_ES_INFO *info)
{
	...
	{
	   printf("MUX_Ps_update_vdp handle %p,info %p\n",handle,info);
	   return HIK_PSMUX_LIB_S_FAIL;
	}

	printf("MUX_Ps_update_vdp stream_mode 0x%x,dscpt_sets 0x%x\n",info->stream_mode,prg->dscpt_sets);
	if (info->stream_mode & INCLUDE_VIDEO_STREAM)
    {
        if (prg->dscpt_sets & INCLUDE_VIDEO_DESCRIPTOR)
		{
