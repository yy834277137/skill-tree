...
EGifOpenFileName(const char *FileName,
    ...
    if (TestExistance){
        //FileHandle = open(FileName, O_WRONLY | O_CREAT | O_EXCL,  S_IREAD | S_IWRITE);
		 FileHandle = open(FileName, O_WRONLY | O_CREAT | O_EXCL,0666);
		printf("egif-70");}
    else
        //FileHandle = open(FileName, O_WRONLY | O_CREAT | O_TRUNC, S_IREAD | S_IWRITE);
		FileHandle = open(FileName, O_WRONLY | O_CREAT | O_TRUNC,0666);
    if (FileHandle == -1) {
...
EGifSetGifVersion(const char *Version) {
    memset(GifVersionPrefix, 0x0, GIF_STAMP_LEN + 1);
    memcpy(GifVersionPrefix, GIF_STAMP, GIF_STAMP_LEN);
 ...
 * follows the GIF file openning.
 æ­¤ä?ç¨‹åº”?¨ä»»ä½•?¶ä?EGifè°ƒ?¨ä??ç«‹?³è??? è·Ÿ?GIF?‡ä»¶æ‰“å¼€?‚
 *****************************************************************************/
int
EGifPutScreenDesc(GifFileType * GifFile,
    ...
    if (Private->FileState & FILE_STATE_SCREEN) {
        //pintf("Private->FileState %d is error.\n",Private->FileState);
        /* If already has screen descriptor - something is wrong! */
        _GifError = E_GIF_ERR_HAS_SCRN_DSCR;
        return GIF_ERROR;
    }
    if (!IS_WRITEABLE(Private)) {
        //pintf("IS_WRITEABLE(Private) is %d.\n",IS_WRITEABLE(Private));
 ...
 * call to any of the pixel dump routines//.?¨å?è¯•è½ *****************************************************************************/
int
