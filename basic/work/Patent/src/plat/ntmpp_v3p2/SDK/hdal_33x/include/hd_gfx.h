...
typedef struct _HD_GFX_IMG_BUF {
	...
	UINTPTR             p_phy_addr[HD_GFX_MAX_PLANE_NUM]; ///< image buffers
...
/**
     parameters for hd_gfx_copy3.
*/
typedef struct _HD_GFX_COPY3 {
	HD_GFX_IMG_BUF          src_img;    ///< source image
	HD_GFX_IMG_BUF          dst_img;    ///< destination image
	HD_IRECT                src_region; ///< which part of source image to copy
	HD_IPOINT               dst_pos;    ///< paste origin
	UINT32                  colorkey_en;///< enable/disable color key
	UINT32                  colorkey;   ///< which source color should be transparent
	UINT32                  alpha;      ///< alpha value of source image
	UINT32                  flush;      ///< flush src/dst buffers' cpu cache
} HD_GFX_COPY3;
...
/**
     parameters for hd_gfx_scale3.
*/
typedef struct _HD_GFX_SCALE3 {
	HD_GFX_IMG_BUF          src_img;    ///< source image
	HD_GFX_IMG_BUF          dst_img;    ///< destination image
	HD_IRECT                src_region; ///< which part of source image to scale
	HD_IRECT                dst_region; ///< paste rectangle
...
/**
     parameters for hd_gfx_rotate3.
*/
typedef struct _HD_GFX_ROTATE3 {
	HD_GFX_IMG_BUF          src_img;    ///< source image
	HD_GFX_IMG_BUF          dst_img;    ///< destination image
	HD_IRECT                src_region; ///< which part of source image to rotate
	HD_IPOINT               dst_pos;    ///< paste origin
...
typedef struct _HD_GFX_COLOR_TRANSFORM {
	HD_GFX_IMG_BUF          src_img;                              ///< source image
	HD_GFX_IMG_BUF          dst_img;                              ///< destination image
	UINTPTR                 p_tmp_buf;                            ///< transformation between certain formats needs extra buffer
	UINT32                  tmp_buf_size;                         ///< length of p_tmp_buf
	UINTPTR                 p_lookup_table[HD_GFX_MAX_PLANE_NUM]; ///< for advancing palette tranform
...
typedef struct _HD_GFX_ARITHMETIC {
	UINTPTR                 p_op1;     ///< array of operands 1
	UINTPTR                 p_op2;     ///< array of operands 2
	UINTPTR                 p_out;     ///< output array
...
/**
    ...
    @return HD_OK for success, other value indicates an error.
*/
void *hd_gfx_memcpy(UINTPTR dest, const UINTPTR src, size_t n);
...
/**
    fill a 2-dimensional buffer with the specific value, alpha effect is ignored
...
/**
    copy two-dimensional images with colorkey and cpu cache flush.

    @param p_param: parameter for copy operation.

    @return HD_OK for success, other value indicates an error.
*/
HD_RESULT hd_gfx_copy3(HD_GFX_COPY3 *p_param);

/**
    rotate an image with cpu cache flush.

    @param p_param: parameter for rotate operation.

    @return HD_OK for success, other value indicates an error.
*/
HD_RESULT hd_gfx_rotate3(HD_GFX_ROTATE3 *p_param);

/**
    scale up/down an image with cpu cache flush.

    @param p_param: parameter for scale operation.

    @return HD_OK for success, other value indicates an error.
*/
HD_RESULT hd_gfx_scale3(HD_GFX_SCALE3 *p_param);
