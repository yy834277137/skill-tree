...
********************************************************************/
#define HD_VIDEOOUT_MAX_IN  64 ///< max count of input of this device (interface)
...
typedef struct _HD_VIDEOOUT_DEVNVR_CONFIG {
	...
	struct {
		...
		UINTPTR buf_paddr;     ///< NVR only. fb buffer physical address
		unsigned int buf_len;       ///< NVR only. fb buffer length
		UINTPTR rle_buf_paddr; ///< NVR only. rle buffer physical address, only valid for gui compress
...
typedef enum _CLEAR_WINDOW_MODE{
	HD_ACTIVE_BY_APPLY = 0,     ///< clear videoout win input data  after hdal module start
...
typedef struct {
	UINT32 *p_video_id;                ///< supported  EDID buffer,user should allocate buffer.
	UINT32 video_id_len;               ///< in:number of p_video_id,out:Number of video formats supported.
	UINT8 *p_edid_data;                ///< EDID data buffer ,user should allocate buffer.
	UINT32 edid_data_len;              ///< in:number of p_edid_data,out:Number of valid edid data.
	UINT32 interface_id;			///< in:from dts interface  id
} HD_VIDEOOUT_EDID_DATA;
...
typedef enum _HD_VIDEOOUT_PARAM_ID {
	...
	HD_VIDEOOUT_PARAM_EDID_DATA,          ///<  support get with ctrl path, using HD_VIDEOOUT_EDID_DATA struct
	///Common End//
