...
unsigned long _vk_raw_spin_lock_irqsave_nested(vk_spinlock_t *p_voslock, int subclass);
...
#define vk_spin_lock_irqsave_nested(p_voslock, flags, subclass) \
do { \
    flags = _vk_raw_spin_lock_irqsave_nested(p_voslock, subclass); \
} while (0)
