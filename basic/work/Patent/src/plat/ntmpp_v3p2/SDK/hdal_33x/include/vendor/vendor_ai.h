...
#define AI_PXLFMT_YUV420_NV21                       0x00000000  ///< YUV420 NV21 (default of 32x and 336)
#define AI_PXLFMT_YUV420_NV12                       0x80000000  ///< YUV420 NV12
#else
...
#define VENDOR_AI_COUNT                             4
#define VENDOR_AI_BASE                              0x4000
#define VENDOR_AI(dev_id)                           (VENDOR_AI_BASE + (dev_id))
#define VENDOR_AI_MAX                               (VENDOR_AI_BASE + VENDOR_AI_COUNT - 1)
#define VENDOR_AI_DEV(dev_id)                       ((VENDOR_AI(dev_id) << 16) | HD_CTRL)
#define VENDOR_AI_PATH(dev_id, proc_id)             ((VENDOR_AI(dev_id) << 16) | ((proc_id) & 0x0000ffff))
...
typedef enum _VENDOR_AI_CFG_ID {
	...
	VENDOR_AI_CFG_PLUGIN_EP_DEVICE    =   8, ///< set    , config EP device pluign
    VENDOR_AI_CFG_MULTI_PROCESS       =   9, ///< set    , support multi process (default 0, set to 0 or 1)
    VENDOR_AI_CFG_LIB_IMPL_VERSION    =  10, ///< get    , lib implement version
	VENDOR_AI_CFG_KFLOW_IMPL_VERSION  =  11, ///< get    , kflow implement version
	VENDOR_AI_CFG_KDRV_IMPL_VERSION   =  12, ///< get    , kdrv implement version
...
/**
	@name struct of ep memory
*/
typedef struct _VENDOR_AI_NET_EP_ADDR {
	uintptr_t pa;                              ///< physical address
	uintptr_t va;                              ///< virtual address
	UINT32 size;                               ///< buffer size
} VENDOR_AI_NET_EP_ADDR;

typedef struct _VENDOR_AI_NET_EP_DMACOPY {
	HD_COMMON_MEM_DDR_ID src_ddr;			 ///< source DDR id
	UINTPTR src_phy_addr;					 ///< source physical address
	HD_COMMON_MEM_DDR_ID dst_ddr; 			 ///< destnation DDR id
	UINTPTR dst_phy_addr;					 ///< destnation physical address
	UINT32 size;							 ///< buffer size, need ALIGN_CEIL_32
} VENDOR_AI_NET_EP_DMACOPY;
...
typedef enum _VENDOR_AI_NET_PARAM_ID {
    ...
    // for EP
    VENDOR_AI_NET_PARAM_MAP_EP_ADDR             = 61, ///< set	  , set address to EP to mmap, using VENDOR_AI_NET_EP_ADDR
    VENDOR_AI_NET_PARAM_UNMAP_EP_ADDR           = 62, ///< set	  , set address to EP to unmap, using VENDOR_AI_NET_EP_ADDR
    VENDOR_AI_NET_PARAM_EP_DMACOPY              = 63, ///< set	  , set address to EP to dmacopy, using VENDOR_AI_NET_EP_DMACOPY
...
extern HD_RESULT vendor_ai_dev_set (UINT32 dev_id, VENDOR_AI_CFG_ID cfg_id, void* p_param);
extern HD_RESULT vendor_ai_dev_get (UINT32 dev_id, VENDOR_AI_CFG_ID cfg_id, void* p_param);
