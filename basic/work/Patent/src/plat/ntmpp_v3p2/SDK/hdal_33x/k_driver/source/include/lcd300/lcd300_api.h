...
typedef enum lcd300_otype {
	...
	LCD300_OTYPE_BT1120_1600x1200x60,
	LCD300_OTYPE_HDMI_3840x2160x60,
	LCD300_OTYPE_64x64,
	LCD300_OTYPE_VGA_1024x600x60,
    LCD300_OTYPE_BT1120_1280x720x25V2,
    LCD300_OTYPE_BT1120_1280x720x30V2,
	LCD300_OTYPE_BT1120_1920x1080x50,
	LCD300_OTYPE_BT1120_1440x900x60,
    LCD300_OTYPE_MAX,
...
typedef struct {
	int wb_en;
    int wb_mode; 	/* 0:discard, 1:Interploation mode, for SDO1 */
	int wb_uvswap;  /* 0: VU, 1: UV */
	int wb_one_shut;/* 0: free-run, 1: one-shut */
	int wb_sel;     /* 0: ch0~ch2+cursor, 1: ch0~ch2, 2: ch0 */
    int wb_lx;   	/* write-back x start pixel (two-pixel alignment), Left x, for SDO1 */
    int wb_ty;   	/* write-back y start line, Top Y, for SDO1 */
    int wb_rx;   	/* write-back x start pixel (two-pixel alignment), Right x, for SDO1 */
    int wb_by;   	/* write-back y start line, Bottom Y, for SDO1 */
	...
	int wb_fmt;     /* 0: 422, 1: 420, 2: 420SCE */
 ...
 * @return 0 on success, <0 on error
 */
int lcd300_drv_set_writeback(int chip_idx, wb_api_t *p_wb);
int lcd300_drv_get_writeback(int chip_idx, wb_api_t *p_wb);
