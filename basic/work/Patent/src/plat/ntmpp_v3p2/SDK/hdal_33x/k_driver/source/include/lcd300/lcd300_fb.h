...
#define LCD300_IOC_SET_WRITEBACK		_IOW(LCD300_IOC_MAGIC, 22, wb_api_t)
...
typedef struct lcd300_wb_xy_limit_t {
	int lx_max; //limit from left
	int ty_max; //limit from top
	int rx_max; //limit from right
	int by_max; //limit from bottom
} lcd300_wb_xy_limit;
...
/* pxclk
*/
#define LCD300_IOC_SET_PXCLK            _IOWR(LCD300_IOC_MAGIC, 54, int)  /*unit: Hz*/
#define LCD300_IOC_GET_PXCLK            _IOR(LCD300_IOC_MAGIC, 55, int)  /*unit: Hz*/

// writeback limit
#define LCD300_IOC_WB_XY_LIMIT          _IOW(LCD300_IOC_MAGIC, 56, lcd300_wb_xy_limit)
