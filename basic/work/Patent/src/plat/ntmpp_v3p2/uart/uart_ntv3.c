...
static Handle g_astUartMutexHandle[UART_ID_BUTT] = {NULL};
...
INT32 UART_Init(UART_ID_E enId, UART_ATTR_S *pstAttr)
{
    ...
    if (g_aiFds[enId] <= 0)
    {
        UART_LOGE("uart[%d] open:%s fail\n", enId, g_aszUartDev[enId]);
        return SAL_FAIL;
    }
    ...
    SAL_mutexCreate(SAL_MUTEX_NORMAL, &g_astUartMutexHandle[enId]);
...
INT32 UART_Read(UART_ID_E enId, UINT8 *pu8Buff, UINT32 u32BufLen, UINT32 *pu32ReadBytes, UINT32 u32TimeOut)
{
   ...
   SAL_mutexLock(g_astUartMutexHandle[enId]);
    fd = g_aiFds[enId];
    if (fd <= 0)
    {
        UART_LOGE("uart[%d] fd[%d] is invalid\n", enId, fd);
      SAL_mutexUnlock(g_astUartMutexHandle[enId]);
        return SAL_FAIL;
    }
    ...
    if (FD_ISSET(fd, &stFds))
    {
        ...
        readSize = read(fd, (void *)&u8Data, 1);
        if(u8Data == 0x5B)
        {
            *pu8Buff++ = u8Data;
            u32Scr = u8Data;
            ...
            readSize = read(fd, (void *)&u8Data, 1);
           // UART_LOGI("uart u8Data[0x%0x]\n", u8Data);
            *pu8Buff++ = u8Data;
            u32Scr += u8Data;
            u32Len = (u8Data & 0xff) << 8;
           ...
           *pu8Buff++ = u8Data;
            ...
            *pu8Buff++ = u8Data;
            ...
            if(u8Data == u32Scr)
            {
                *pu32ReadBytes = (UINT32)u32Len + 5;
            }
            else
            {
                UART_LOGE("uart crs[0x%0x] != u8Data[0x%0x] %ld\n", u32Scr, u8Data, readSize);
                SAL_mutexUnlock(g_astUartMutexHandle[enId]);
                return SAL_FAIL;
            }

            *pu32ReadBytes = (UINT32)u32Len;
