...
#define ICF_VERSION_YEAR             2021
#define ICF_VERSION_MONTH            12
#define ICF_VERSION_DAY              16
...
/** @fn  int ICF_Init
 *  @brief  <引擎初始化>
 *  @param  pInitInParam      [in]          - 输入参数结构体 ICF_INIT_PARAM
 *  @param  nInitInParamSize  [in]          - 输入参数大小
 *  @param  pInitOutParam     [out]         - 输出参数结构体 ICF_INIT_HANDLE
...
/** @fn  int ICF_Finit
 *  @brief  <引擎反始化>
 *  @param  pFinitInParam     [in]          - 输入参数结构体 ICF_INIT_HANDLE
 *  @param  nFinitInParamSize [in]          - 输入参数大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_Finit(void                *pFinitInParam,
              unsigned int         nFinitInParamSize);

/** @fn  int ICF_LoadModel
 *  @brief  <预加载模型>
 *  @param  pLMInParam         [in]          - 输入参数结构体 ICF_CREATE_PARAM
 *  @param  nLMInParamSize     [in]          - 输入参数大小
 *  @param  pLMOutParam        [out]         - 输出参数结构体 ICF_MODEL_HANDLE
 *  @param  nLMOutParamSize    [in]          - 输出参数大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_LoadModel(void          *pLMInParam,
 ...
 *  @brief  <模型卸载>
 *  @param  pULMInParam      [in]          - 输入参数结构体 ICF_MODEL_HANDLE
 *  @param  nULMInParamSize  [in]          - 输入参数大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_UnloadModel(void        *pULMInParam,
                    unsigned int nULMInParamSize);

/** @fn  int ICF_Create
 *  @brief  <引擎创建>
 *  @param  pCreateParam         [in]          - 输入参数结构体 ICF_CREATE_PARAM
 *  @param  nCreateParamSize     [in]          - 输入参数大小
 *  @param  pCreateHandle        [out]         - 输出handle ICF_CREATE_HANDLE
 *  @param  nHandleSize          [in]          - 结构体大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_Create(void           *pCreateParam,
 ...
 *  @brief  <引擎分析数据输入>
 *  @param  pChannelHandle     [in]   - ICF_CREATE_HANDLE.pChannelHandle，代表应用中某一路
 *  @param  pInputData         [in]   - 分析数据
 *  @param  nDataSize          [in]   - 分析数据大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
*/
int ICF_InputData(void            *pChannelHandle,
                  void            *pInputData,
                  unsigned int     nDataSize);

/** @fn  int ICF_SetConfig
 *  @brief  <引擎配置>
 *  @param  pChannelHandle   [in]          - ICF_CREATE_HANDLE.pChannelHandle
 *  @param  pConfigParam     [in]          - 引擎配置参数结构体 ICF_CONFIG_PARAM
 *  @param  nConfigParamSize [in]          - 参数大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_SetConfig(void          *pChannelHandle,
                  void          *pConfigParam,
 ...
 *  @brief  <获取引擎配置>
 *  @param  pChannelHandle   [in]          - ICF_CREATE_HANDLE.pChannelHandle
 *  @param  pConfigParam     [in/out]      - 引擎配置参数结构体 ICF_CONFIG_PARAM
 *  @param  nConfigParamSize [out]         - 参数大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_GetConfig(void          *pChannelHandle,
                  void          *pConfigParam,
                  unsigned int   nConfigParamSize);
 ...
 *  @brief  <获取引擎配置>
 *  @param  pChannelHandle  [in]          - ICF_CREATE_HANDLE.pChannelHandle
 *  @param  pCBParam        [in]          - 回调参数结构体 ICF_CALLBACK_PARAM
 *  @param  nCBParamSize    [in]          - 参数大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_SetCallback(void         *pChannelHandle,
                    void         *pCBParam,
 ...
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
*/
int ICF_SubFunction(void            *pChannelHandle,
                    void            *pInputData,
 ...
 *  @brief  <引擎配置ICF框架参数>
 *  @param pInitHandle           -I    ICF句柄 ICF_INIT_HANDLE.pInitHandle
 *  @param nKey                  -I    参数key ICF_KEY_CONFIG
 *  @param pParam                -I    参数
 *  @param nSize                 -I    参数大小
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_System_SetConfig(void *pInitHandle,
                         int   nKey,
                         void *pParam,
                         int   nSize);
 ...
 *  @return 在入参为空时确保返回NULL错误码，入参中的参数为空则可能由于参数不存在或则非法而返回参数错误或则NULL错误
 */
int ICF_System_GetConfig(void *pInitHandle,
                         int   nKey,
                         void *pParam,
                         int   nSize);
