...
typedef struct _ICF_MEM_BUFFER_
{
    ...
    int                reserved[3];                 /// 预留字段
}ICF_STR_ALIGN_4 ICF_MEM_BUFFER;
...
*/
typedef struct _ICF_MEM_INTERFACE_
{
    ...
    int                reserved[4];                 /// 预留字段
}ICF_STR_ALIGN_4 ICF_MEM_INTERFACE;
...
{  
    ...
    int                reserved[4];                 /// 预留字段
}ICF_STR_ALIGN_4 ICF_CONFIG_INFO;

/// 引擎初始化结构体 ICF_Init传入参数
typedef struct _ICF_INIT_PARAM_
{
    ICF_MEM_INTERFACE     stMemConfig;                 /// 内存接口配置
    ICF_CONFIG_INFO       stConfigInfo;                /// 配置信息
    void                 *pScheduler;                  /// 调度器(只做透传,不做校验)
    int                   reserved[4];                 /// 预留字段
}ICF_STR_ALIGN_4 ICF_INIT_PARAM;

/// ICF初始化输出参数
typedef struct _ICF_INIT_HANDLE_
{
    void                 *pInitHandle;                 /// 初始化句柄
    int                   reserved[4];                 /// 预留字段
}ICF_STR_ALIGN_4 ICF_INIT_HANDLE;


/// HVA创建参数
typedef struct _ICF_HVA_PARAM_INFO_
{
    int                 nAlgNums;                                                     // 算子个数
...
}ICF_STR_ALIGN_4 ICF_HVA_PARAM_INFO;
...
typedef struct _ICF_APP_PARAM_INFO_
{
    ...
    ICF_BUFF             stAppAlgModelBuff;               /// 用于底层算子读取模型文件的缓存(透传给用户)
    ICF_HVA_PARAM_INFO  *pHvaParamInfo;                   /// HVA算子创建参数; 可为空,按照默认方式赋值
}ICF_STR_ALIGN_4 ICF_APP_PARAM_INFO;

/// 内存统计参数
typedef struct _ICF_MEMSIZE_PARAM_
{
    ICF_MEM_INFO    stNonSharedMemInfo[ICF_MEM_TYPE_NUM];           // 通道独占内存
    ICF_MEM_INFO    stSharedMemInfo[ICF_MEM_TYPE_NUM];              // 通道共享内存
    ICF_MEM_INFO    stMemScratchSize[ICF_MEM_TYPE_NUM];             // 可复用内存表(暂时不提供，后续完善)
    int             nReversed[16];
}ICF_STR_ALIGN_4 ICF_MEMSIZE_PARAM;

/// 模型创建参数
typedef struct _ICF_MODEL_PARAM_
{
    void                      *pInitHandle;                          // 初始化句柄 ICF_INIT_HANDLE.pInitHandle
    int                        nGraphType;                           // 计算图类型
    int                        nGraphID;                             // 计算图ID
    const char                *pGraphName;                           // 计算图名称
    ICF_APP_PARAM_INFO        *pAppParam;                            // APP传递给算子的参数
    ...
    int                        reserved[10];
}ICF_STR_ALIGN_4 ICF_MODEL_PARAM;

/// 模型句柄结构体
typedef struct _ICF_MODEL_HANDLE_
{
    void                      *pModelHandle;                         // 创建的模型句柄
    ICF_MEMSIZE_PARAM          modelMemSize;                         // 创建模型消耗的内存
    int                        reserved[10];
}ICF_STR_ALIGN_4 ICF_MODEL_HANDLE;

/// 计算图创建参数
typedef struct _ICF_CREATE_PARAM_
{
    ICF_MODEL_PARAM            modelParam;                           // 模型参数
    ICF_MODEL_HANDLE           modelHandle;                          // 模型句柄
    int                        reserved[10];
}ICF_STR_ALIGN_4 ICF_CREATE_PARAM;

/// 计算图创建句柄
typedef struct _ICF_CREATE_HANDLE_
{
    void                      *pChannelHandle;                       // 通道句柄
    ICF_MEMSIZE_PARAM          createMemSize;                        // 创建算子和运行时需要消耗的内存大小
    int                        reserved[10];
}ICF_STR_ALIGN_4 ICF_CREATE_HANDLE;

/// 回调参数结构体
typedef struct _ICF_CALLBACK_PARAM_
{
    ...
    void              *pReserved[2];                /// 预留指针字段
    int                nReserved[2];                /// 预留参数字段
}ICF_STR_ALIGN_4 ICF_CALLBACK_PARAM;

/// 节点配置参数结构体
typedef struct _ICF_CONFIG_PARAM_
{
    int                nNodeID;                     /// 设置回调的节点ID
    int                nKey;                        /// 配置关键字
    ...
    void              *pReserved[2];                /// 预留指针字段
    int                nReserved[1];                /// 预留参数字段
}ICF_STR_ALIGN_4 ICF_CONFIG_PARAM;
...
}ICF_STR_ALIGN_4 ICF_SYSTEMTIME;
...
typedef struct _ICF_MEDIA_RAW_DATA_
{
    ...
    void              *pReserved[2];                /// 预留指针字段
    int                nReserved[2];                /// 预留参数字段
}ICF_STR_ALIGN_4 ICF_MEDIA_RAW_DATA;
...
typedef struct _ICF_MEDIA_DATA_
{
    ...
    void              *pReserved[2];                /// 预留指针字段
    int                nReserved[2];                /// 预留参数字段
}ICF_STR_ALIGN_4 ICF_MEDIA_DATA;
...
typedef struct _ICF_DECODE_DATA_
{
    ...
    void              *pReserved[2];                    /// 预留指针字段
    int                nReserved[2];                    /// 预留参数字段
}ICF_STR_ALIGN_4 ICF_DECODE_DATA;
...
typedef struct _ICF_SOURCE_BLOB_
{
    ...
    void                     *pReserved[4];                   /// 预留指针字段
    int                       nReserved[4];                   /// 预留参数字段
} ICF_STR_ALIGN_4 ICF_SOURCE_BLOB;
...
typedef struct _ICF_INPUT_DATA_
{
    ...
    void              *pReserved[2];                      /// 预留指针字段
    int                nReserved[2];                      /// 预留参数字段
}ICF_STR_ALIGN_4  ICF_INPUT_DATA;
...
typedef struct _ICF_MEDIA_INFO_
{
    ...
    void              *pReserved[2];                      /// 预留指针字段
    int                nReserved[2];                      /// 预留参数字段
}ICF_STR_ALIGN_4 ICF_MEDIA_INFO;

/// 用于统一包装算子输出数据或源数据
typedef struct ICF_DATA_PACK_
{
...
typedef struct _ICF_NODE_LABEL_
{
    ...
    void              *pReserved[2];                      /// 预留指针字段
    int                nReserved[2];                      /// 预留参数字段
}ICF_STR_ALIGN_4 ICF_NODE_LABEL;
...
typedef struct _ICF_MEMPOOL_STATUS_
{
    ...
    ICF_MEM_TYPE_E      nSpace;                           /// 内存类型
} ICF_STR_ALIGN_4 ICF_MEMPOOL_STATUS;
