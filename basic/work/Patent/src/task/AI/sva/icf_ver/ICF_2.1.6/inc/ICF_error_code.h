...
#define ICF_MEMPOOL_ERR_HISI_SYS_VB               (0x88001013) // 海思接口分配VB类型内存失败
#define ICF_MEMPOOL_ERR_NOTMATCH                  (0x88001014) // 内存池和通道不匹配
#define ICF_MEMPOOL_ERR_NOTEXIST                  (0x88001015) // 不存在
#define ICF_MEMPOOL_ERR_TYPE_ERROR                (0x88001016) // 内存池内存类型不一致，一般要求HVA序列化、反序列化算子结果内存类型相同
...
#define ICF_E_SCHD_POSIX                          (0x8800200B) // POSIX错误
...
#define ICF_E_SCHD_NULL                           (0x88002010) // 空指针

// GRAPH层错误码
#define ICF_E_GRAPH_BASE                          (0x88003000) // NODE节点管理错误码
#define ICF_E_GRAPH_NULL                          (0x88003001) // 指针为空
#define ICF_E_GRAPH_DESTROY                       (0x88003002) // GRAPH销毁时失败
#define ICF_E_GRAPH_SCHED_NULL                    (0x88003003) // ICF调度器句柄为空
#define ICF_E_GRAPH_START_MODEL                   (0x88003004) // ICF以本地模式启动但是没有计算图
#define ICF_E_GRAPH_PKG_RELEASE                   (0x88003005) // 数据包释放
#define ICF_E_GRAPH_SET_REMOTE                    (0x88003006) // 非主GRAPH配置了远程节点
#define ICF_E_GRAPH_GET_REMOTE                    (0x88003007) // 非主GRAPH获取了远程节点的配置
#define ICF_E_GRAPH_MGR_INIT                      (0x88003008) // 图管理器初始化失败
#define ICF_E_GRAPH_ORG_CREATE                    (0x88003009) // 创建原始图失败
#define ICF_E_GRAPH_EXE_CREATE                    (0x8800300A) // 创建执行图失败
#define ICF_E_GRAPH_PARAM                         (0x8800300B) // 参数错误
#define ICF_E_GRAPH_NODE_EXIST                    (0x8800300C) // 节点不存在
#define ICF_E_GRAPH_VERTEX_GET_NODE               (0x8800300D) // 创建节点失败

#define ICF_E_GRAPH_CREATOBJ                      (0x88003010) // 创建对象失败
#define ICF_E_GRAPH_ALG_NOT_EXIST                 (0x88003011) // 算子不存在
#define ICF_E_GRAPH_NOTEXIT                       (0x88003012) // 不存在
#define ICF_E_GRAPH_SUPPORT                       (0x88003013) // 不支持
#define ICF_E_GRAPH_REPETID                       (0x88003014) // 重复ID
#define ICF_E_GRAPH_MALLOC                        (0x88003015) // 申请内存失败
#define ICF_E_GRAPH_EXCEED                        (0x88003016) // 路数超过
#define ICF_E_GRAPH_REINIT                        (0x88003017) // 重复初始化
#define ICF_E_GRAPH_NOINITE                       (0x88003018) // 未初始化
#define ICF_E_GRAPH_NOTSUPPORT                    (0x88003019) // 不支持
#define ICF_E_GRAPH_LACK_RESOURCE                 (0x8800301A) // 资源不足
#define ICF_E_GRAPH_JSONPARAM                     (0x8800301B) // TASK.json配置文件参数异常
#define ICF_E_GRAPH_JSON_ALGPARAM                 (0x8800301C) // alg.json配置文件参数异常
#define ICF_E_GRAPH_INPUTPORT                     (0x8800301D) // 输入端口数超出最大端口数
#define ICF_E_GRAPH_NOTSUPPORTSHARE               (0x8800301F) // 合并Node不支持共享
#define ICF_E_GRAPH_NEEDPORTS                     (0x88003020) // 合并Node需要端口
#define ICF_E_GRAPH_NODESIDE                      (0x88003021) // NODE端配置出错
#define ICF_E_GRAPH_JSONREADCONFIG                (0x88003022) // TASK.json文件读取失败
#define ICF_E_GRAPH_GBUFTYPEREPETID               (0x88003023) // TASK.json配置中全局缓存内存类型重复
#define ICF_E_GRAPH_JSONGBUFPARAM                 (0x88003024) // TASK.json配置中全局缓存内存配置错误
#define ICF_E_GRAPH_JSONTASKTYPE                  (0x88003025) // TASK.json配置中TaskType不支持
#define ICF_E_GRAPH_JSONNODEPARAM                 (0x88003026) // TASK.json配置中节点NODE参数异常
#define ICF_E_GRAPH_JSONCONNECTPARAM              (0x88003027) // TASK.json配置中流水连接CONNECT参数异常
#define ICF_E_GRAPH_GRAPH_EVENT                   (0x88003028) // 事件控制不属于同一个通道
#define ICF_E_GRAPH_DECODE_NUM                    (0x88003029) // 同一通道多个解码节点
#define ICF_E_GRAPH_NODE_INPUT                    (0x88003030) // 没有找到输入目标节点(如不存在解码节点)
#define ICF_E_GRAPH_NODE_MEM_TYPE_EXCEED          (0x88003031) // 内存种类数量超出范围
#define ICF_E_GRAPH_WRONG_JSON_PATH               (0x88003032) // JSON文件路径错误，无法打开
#define ICF_E_GRAPH_SHORT_OF_PKG                  (0x88003033) // 数据包不足
#define ICF_E_GRAPH_EXISTED                       (0x88003034) // 已存在
#define ICF_E_GRAPH_GRAPH_FIND                    (0x88003035) // 未能查找到相关计算图，一般是nGraphType或nGraphId不存在
#define ICF_E_GRAPH_DEV_CONNECT                   (0x88003036) // 分布式下计算图层设备连接失败，获取不到通信通道
#define ICF_E_GRAPH_DEV_INFO                      (0x88003037) // 设备地址或服务初始化失败
#define ICF_E_GRAPH_APP_REGISTER                  (0x88003038) // APP信息注册失败

#define ICF_E_API_CREATE_HANDLE                   (0x88003C00) // 对外接口层创建句柄失败
...
#define ICF_E_NODEEXEC_METADATA_PACK              (0x88004019) // METADATA组装出错
...
#define ICF_E_PACKAGE_INPUTDAT_PARAM              (0x88004F05) // InputData()输入数据参数异常
#define ICF_E_PACKAGE_NOT_EXIST                   (0x88004F06) // 数据包不存在
#define ICF_E_PACKAGE_REBACK                      (0x88004F06) // 数据包重复归还
...
#define ICF_ALG_ALGTYPE                           (0x8800500E) // 错误的算子类型
// 算子层错误码-算子池
...
#define ICF_ALGP_DFR_COMPARE                      (0x88005a08) // DFR比对错误
#define ICF_ALGP_DFR_LIVENESS                     (0x88005a09) // DFR活体错误
#define ICF_ALGP_PROCESS_RETURN                   (0x88005a10) // 并非是错误，而是不再继续处理(hva模式在前后处理中使用)
...
#define ICF_TOOL_PACKAFE_NOTFOUND                 (0x88007006) // 数据包未找到
#define ICF_TOOL_TRAN_TYPE_ERR                    (0x88007007) // 类型转换失败
#define ICF_TOOL_PARAM                            (0x88007008) // 参数错误
...
// RPC层错误码
#define ICF_DIST_RPC_BASE                         (0x8800B000)                // PRC错误码基址
#define ICF_DIST_RPC_NULL                         (0x8800B001)  // 空指针
#define ICF_DIST_RPC_COMBETYPE                    (0x8800B002)  // 通信标准错误
#define ICF_DIST_RPC_PARAM                        (0x8800B003)  // 参数错误
#define ICF_DIST_RPC_NOT_INITED                   (0x8800B004)  // 尚未初始化
#define ICF_DIST_RPC_SOCKET_CREATE                (0x8800B005)  // socket创建失败
#define ICF_DIST_RPC_SOCKET_EVENT                 (0x8800B006)  // socket事件创建失败
#define ICF_DIST_RPC_SOCKET_COMMU                 (0x8800B007)  // socket通信失败
#define ICF_DIST_RPC_SOCKET_DESTORY               (0x8800B008)  // socket销毁失败
#define ICF_DIST_RPC_SERVER_INIT                  (0x8800B009)  // server初始化失败
#define ICF_DIST_RPC_REPRATED                     (0x8800B00A)  // 重复
#define ICF_DIST_RPC_SEND                         (0x8800B00B)  // RPC请求发送失败
#define ICF_DIST_RPC_NOTFINISH                    (0x8800B00C)  // 未完成
#define ICF_DIST_RPC_IPC_CREATE                   (0x8800B00D)  // IPC创建失败
#define ICF_DIST_RPC_IPC_DESTORY                  (0x8800B00E)  // IPC销毁失败
#define ICF_DIST_RPC_IPC_COMMU                    (0x8800B00F)  // IPC通信失败
#define ICF_DIST_RPC_SOCKET_MSG                   (0x8800B010)  // Socket消息不符合要求

// 代理服务层错误码
#define ICF_DIST_SERVICE_BASE                     (0x8800C000)                    // 代理服务错误码基址
#define ICF_DIST_SERVICE_NULL                     (0x8800C001)  // 空指针
#define ICF_DIST_SERVICE_REQUEST_PARAM            (0x8800C002)  // 参数错误
#define ICF_DIST_SERVICE_NO_CHANNEL               (0x8800C003)  // 无法获取连接通道
#define ICF_DIST_SERVICE_NO_MEMORY                (0x8800C004)  // 没有内存供申请
#define ICF_DIST_SERVICE_SERVER_EXIST             (0x8800C005)  // 该服务不存在
#define ICF_DIST_SERVICE_SERVER_NOT_START         (0x8800C006)  // 服务器未启动
#define ICF_DIST_SERVICE_SERVICE_EXIST            (0x8800C007)  // 服务已经存在
#define ICF_DIST_SERVICE_SERVICE_NOT_EXIST        (0x8800C008)  // 服务不存在
#define ICF_DIST_SERVICE_NOT_SUPPORT              (0x8800C009)  // 服务不支持
#define ICF_DIST_APP_LAUNCH_WRITE_FILE_SIZE       (0x8800C00A)  // 文件写的大小不正确
#define ICF_DIST_APP_LAUNCH_CREATE_DIR            (0x8800C00B)  // 创建目录失败
#define ICF_DIST_FILE_OPEN                        (0x8800C00C)  // 文件打开失败
#define ICF_DIST_RPC_STATUE                       (0x8800C00D)  // rpc状态错误
#define ICF_DIST_SERVICE_ALLOC_RESOURCE           (0x8800C00E)  // 分配资源失败

// 全局缓存模块
#define ICF_GLOBAL_CACHE_ERR_BASE                 (0x8800D000)
#define ICF_GLOBAL_CACHE_ERR_MEM_SIZE             (0x8800D001)    // 全局缓存配置的大小小于0
#define ICF_GLOBAL_CACHE_ERR_MEM_LIMIT            (0x8800D002)    // 全局缓存总内存超出配置的限制
#define ICF_GLOBAL_CACHE_CMD_NOT_SUPPORT          (0x8800D003)    // 命令不支持
#define ICF_GLOBAL_CACHE_KEY_NOT_EXIST            (0x8800D004)    // 缓存关键字不存在
#define ICF_GLOBAL_CACHE_LOCK_FAILED              (0x8800D005)    // 缓存锁定失败
#define ICF_GLOBAL_CACHE_INPUT_PARAM              (0x8800D006)    // 全局缓存接口输入参数错误
#define ICF_GLOBAL_CACHE_NOT_CFG                  (0x8800D007)    // 通道的全局缓存未配置
#define ICF_GLOBAL_CACHE_ERR_MEM_TYPE             (0x8800D008)    // 通道的全局缓存类型错误
#define ICF_GLOBAL_CACHE_MASTER_ADDRESS           (0x8800D009)    // 获取全局缓存Master地址
#define ICF_GLOBAL_CACHE_CREATE_NOT_CONSIST       (0x8800D00A)    // 同一Key多次创建不一致
#define ICF_GLOBAL_CACHE_LOCAL_ADDRESS            (0x8800D00B)    // 目标地址是本地地址
#define ICF_GLOBAL_CACHE_MGR_NULL                 (0x8800D00C)    // 本地的全局缓存管理器为空
#define ICF_GLOBAL_CACHE_SIZE                     (0x8800D00D)    // 本地的全局缓存块大小小于写入大小
#define ICF_GLOBAL_CACHE_INTERFACE_NOT_SUPPORT    (0x8800D00E)    // 强弱一致性接口混用
#define ICF_GLOBAL_CACHE_FETCH_DATA_NULL          (0x8800D00F)    // fetch data 返回空
#define ICF_GLOBAL_CACHE_FETCH_DATA_SIZE          (0x8800D010)    // fetch data 大小和本地不一致

// 依赖部署模块
#define ICF_DEPENDENT_WITHOUT_DEPLOY              (0x8800E001)    // 无需部署
