...
static INT32 icf_engine_deinit_InnerChn(VOID)
{
	...
	memset(&g_stIcfInnerInfo, 0x00, sizeof(ICF_INNER_INFO));
    return s32Ret;
}
...
static INT32 icf_engine_destroy(VOID)
{
	...
	s32Ret = SAL_SOK;
	SAL_LOGI("icf engine destroy end! \n");
...
static INT32 icf_engine_create(const ENGINE_COMM_INIT_PRM_S *pstInitPrm, ENGINE_COMM_INIT_OUTPUT_PRM *pstInitOutput)
{
    ...
    for (i = 0; i < g_stIcfInnerInfo.u32ChnNum; i++)
    {
        ...
        {
            memcpy(&stCreateParam.modelParam, &g_stIcfInnerInfo.stModelParam[i], sizeof(ICF_MODEL_PARAM));
            memcpy(&stCreateParam.modelHandle, &g_stIcfInnerInfo.stModelHandle[i], sizeof(ICF_MODEL_HANDLE));
...
static INT32 icf_engine_unload_model(VOID)
{
	...
	for(i = 0; i < 2; i++)
	{
		if (!g_stIcfInnerInfo.stModelHandle[i].pModelHandle)
		{
			SAL_LOGW("model handle is destroyed! return success! i %d \n", i);
			continue;
		}

	    s32Ret = g_stIcfLibInfo.stIcfLibApi.IcfUnloadModel(&g_stIcfInnerInfo.stModelHandle[i], sizeof(ICF_MODEL_HANDLE));
	    if (SAL_SOK != s32Ret)
	    {
	...
	s32Ret = SAL_SOK;
	SAL_LOGI("icf engine unload model end! \n");
exit:
    return s32Ret;
}
...
static INT32 icf_engine_load_model(const ENGINE_COMM_INIT_PRM_S *pstInitPrm)
{
    ...
    for (i = 0; i < pstInitPrm->stInitChnCfg.u32ChnNum; i++)
    {

        g_stIcfInnerInfo.stAppParam[i].pAppParamCfgPath = pstInitPrm->stInitChnCfg.astChnAttr[i].pAppParamCfgPath;
        /* g_stIcfInnerInfo.stAppParam[1].pAppParamCfgPath = pstInitPrm->stInitChnCfg.astChnAttr[0].pAppParamCfgPath; */

        g_stIcfInnerInfo.stModelParam[i].nGraphID = pstInitPrm->stInitChnCfg.astChnAttr[i].u32GraphId;  /* szl_todo: 入参传入 */
        g_stIcfInnerInfo.stModelParam[i].nGraphType = pstInitPrm->stInitChnCfg.astChnAttr[i].u32GraphType; /*1*/		/* fixme: graphtype 1表示视频模式，2表示仅分包 */
        g_stIcfInnerInfo.stModelParam[i].pAppParam = &g_stIcfInnerInfo.stAppParam[i];
        g_stIcfInnerInfo.stModelParam[i].nMaxCacheNums = 0;
        g_stIcfInnerInfo.stModelParam[i].pInitHandle = g_stIcfInnerInfo.stIcfHandle.pInitHandle;
        s32Ret = g_stIcfLibInfo.stIcfLibApi.IcfLoadModel(&g_stIcfInnerInfo.stModelParam[i],
                                                         sizeof(ICF_MODEL_PARAM),
                                                         &g_stIcfInnerInfo.stModelHandle[i],
                                                         sizeof(ICF_MODEL_HANDLE));

        if (SAL_SOK != s32Ret)
        {
            SVA_LOGE("Icf IcfLoadModel Failed! ret: 0x%x. \n", s32Ret);
            goto exit;
        }

        SAL_LOGI("----------chan %d mode load ok!!-----------\n", i);
        icf_pr_model_mem_info(&g_stIcfInnerInfo.stModelHandle[i]);
    }
...
static INT32 icf_engine_deinit(VOID)
{
	...
	if (!g_stIcfInnerInfo.stIcfHandle.pInitHandle)
	{
		SAL_LOGW("icf handle is destroyed! return success! \n");
		s32Ret = SAL_SOK;
	...
	SAL_LOGI("icf engine deinit end! \n");
...
static INT32 icf_rslv_icf_lib_sym(VOID)
{
	...
	if (g_stIcfLibInfo.pIcfLibHandle)
	{
		SAL_LOGW("icf lib has been loaded! return success! \n");
		return SAL_SOK;
	}
    ...
    /* ICF_Interface.h */
    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_Init_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfInit);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_Init failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_Finit_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfFinit);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_Finit failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_LoadModel_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfLoadModel);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_Init failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_UnloadModel_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfUnloadModel);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_Finit failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_Create_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfCreate);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_Create failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_Destroy_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfDestroy);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_Destroy failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_SetConfig_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfSetConfig);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_SetConfig failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_GetConfig_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfGetConfig);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_GetConfig failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_SetCallback_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfSetCallback);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_Set_callback failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_GetVersion_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfGetVersion);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_GetVersion failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_InputData_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfInputData);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_InputData failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_Package_GetState_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfGetPackageStatus);
    ENGINE_HAL_CHECK_RET(SAL_SOK != s32Ret, exit, "Sal_GetLibSymbol ICF_Package_GetState failed!");

    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pIcfLibHandle, "ICF_GetDataPtrFromPkg_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfGetPackageDataPtr);
...
static INT32 icf_rslv_xsie_lib_sym(VOID)
{
	...
	if (g_stIcfLibInfo.pXsieLibHandle)
	{
		SAL_LOGW("xsie lib has been loaded! return success! \n");
		return SAL_SOK;
	}
    ...
    /* ICF_Interface.h */
    s32Ret = Sal_GetLibSymbol(g_stIcfLibInfo.pXsieLibHandle, "ICF_APP_GetVersion_V2", (VOID **)&g_stIcfLibInfo.stIcfLibApi.IcfAppGetVersion);
                               ...
                               ENGINE_COMM_OUTPUT_DATA_S *pstOutputRslt)
{
...
#if 0  /* debug: 模拟引擎内部处理异常 */
	SAL_LOGE("get user info failed! \n");
	goto exit;
#endif
    ...
    if (pstUsrPrvtCtrlPrm->bUsed)
    {
        ...
        s32Ret = pstOutputRslt->stOutputCtlPrm.pFuncGetUserInfo(pstUsrPrvtCtrlPrm->pUsrData,
                                                               pstOutputRslt->stOutputCtlPrm.pUserInfo);
 ...
 * @brief      获取pkg输出状态
 * @param[in]  VOID *pstRawOutput                                        
 * @param[in]  ENGINE_COMM_USER_PRVT_CTRL_PRM **ppstUsrPrvtCtrlPrm       
 * @param[in]  ENGINE_COMM_ENGINE_PRVT_CTRL_PRM **ppstEnginePrvtCtrlPrm  
 * @param[out] None
 * @return     static INT32
 */
static INT32 icf_get_pkg_status(VOID *pstRawOutput,
                                ENGINE_COMM_USER_PRVT_CTRL_PRM **ppstUsrPrvtCtrlPrm,
                                ENGINE_COMM_ENGINE_PRVT_CTRL_PRM **ppstEnginePrvtCtrlPrm)
{
    INT32 s32Ret = SAL_FAIL;

    ICF_MEDIA_INFO *pstInputYuvData = NULL;
	ENGINE_COMM_INPUT_DATA_USER_INFO *pstInputUsrPrvtInfo = NULL;

    s32Ret = g_stIcfLibInfo.stIcfLibApi.IcfGetPackageStatus(pstRawOutput);
    ENGINE_HAL_CHECK_RET_NO_LOOP(s32Ret, "icf get pkg status failed!");

#if 0  /* debug: 模拟引擎内部处理异常 */
	s32Ret = SAL_FAIL;
	SAL_LOGE("simulate engine proc failed! \n");
#endif
    ...
    /* 若用户私有参数有效，则进行输出 */
    pstInputUsrPrvtInfo = (ENGINE_COMM_INPUT_DATA_USER_INFO *)pstInputYuvData->stInputInfo.pUserPtr;

	/* 用户透传数据 */
    *ppstUsrPrvtCtrlPrm = &pstInputUsrPrvtInfo->stUsrPrvtCtrlPrm;
	
	/* engine层透传数据 */
    *ppstEnginePrvtCtrlPrm = &pstInputUsrPrvtInfo->stEnginePrvtInfo;
...
static INT32 engine_comm_get_alg_result(VOID *pstOutput, ENGINE_COMM_OUTPUT_DATA_S *pstOutputRslt)
{
    ...
    ENGINE_COMM_USER_PRVT_CTRL_PRM *pstUserPrvtCtrlPrm = NULL;
    ENGINE_COMM_ENGINE_PRVT_CTRL_PRM *pstEngPrvtCtrlPrm = NULL;

    /* checker */
    ENGINE_HAL_CHECK_RET(NULL == pstOutput, rls, "pstOutput == null!");
    ENGINE_HAL_CHECK_RET(NULL == pstOutputRslt, rls, "pstOutputRslt == null!");
	
    /* 获取数据包状态 */
    s32Ret = icf_get_pkg_status(pstOutput, &pstUserPrvtCtrlPrm, &pstEngPrvtCtrlPrm);
    ENGINE_HAL_CHECK_RET(s32Ret, rls, "icf_get_user_info failed!");

    /* 获取用户透传信息 */
    s32Ret = icf_get_user_info(pstOutput, pstOutputRslt);
    ENGINE_HAL_CHECK_RET(s32Ret, rls, "icf_get_user_info failed!");
...
rls:
	/* 释放外部用户送入的私有数据 */
	if (pstUserPrvtCtrlPrm && pstUserPrvtCtrlPrm->pFuncRlsUserPrvtInfo)
	{
		(VOID)pstUserPrvtCtrlPrm->pFuncRlsUserPrvtInfo(pstUserPrvtCtrlPrm->pUsrData);
	}
...
static INT32 engine_comm_input_data(ENGINE_COMM_INPUT_DATA_INFO *pstIcfCommInputDataInfo)
{
	...
	ENGINE_COMM_INPUT_DATA_USER_INFO *pstInputUserInfo = NULL;
	...
	pstInputUserInfo = &pstIcfCommInputDataInfo->stInputUserInfo;

    /* 填充引擎框架内部使用的全局缓存信息，注意在回调处理时需要释放 */
    pstInputUserInfo->stEnginePrvtInfo.pEngineInnerChnPrvtData = (VOID *)pstInnerChnBuf;
    pstInputUserInfo->stEnginePrvtInfo.pFuncProcEngPrvtData = (ENGINE_COMM_ENGINE_PROC_PRVT_DATA)icf_engine_rls_inner_chn_buf;

    pstIcfInputData->pUserPtr = (VOID *)pstInputUserInfo;
	...
	if (s32Ret)
	{
		for (i = 0; i < pstIcfInputData->nBlobNum; i++)
		{
			if (!pstIcfInputData->pUseFlag[i])
			{
				SAL_LOGE("pUseFlag null! i %d, nBlobNum %d \n", i, pstIcfInputData->nBlobNum);
				continue;
			}
			
			*(pstIcfInputData->pUseFlag[i]) = SAL_FALSE;
		}
	}
exit:
	/* 释放外部用户送入的私有数据 */
	if (s32Ret)
	{
		if (pstInputUserInfo && pstInputUserInfo->stUsrPrvtCtrlPrm.pFuncRlsUserPrvtInfo)
		{
			pstInputUserInfo->stUsrPrvtCtrlPrm.pFuncRlsUserPrvtInfo(pstInputUserInfo->stUsrPrvtCtrlPrm.pUsrData);
		}
	}
    return s32Ret;
}
