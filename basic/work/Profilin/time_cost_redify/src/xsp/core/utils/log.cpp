...
LogLevel g_LogSaveMode = LOG_LEVEL_PRINT_SAVE;
LOG_LEVEL g_LogLevel = LOG_INFO;
namespace xlog
{
...
static const char *level_strings[] = {
  "[ALG_TRACE]", "[ALG_DEBUG]", "[ALG_INFO]", "[ALG_WARN]", "[ALG_ERROR]", "[ALG_FATAL]"
};
...
static void checkout_callback(log_Event *ev) {
	char buf[16];
	buf[strftime(buf, sizeof(buf), "%H:%M:%S", ev->time)] = '\0';
#ifdef LOG_USE_COLOR
	fprintf(
...
#else
	fprintf(
		(FILE*)(ev->udata), "%s %-5s %s:%d %s. ",
		buf, level_strings[ev->level], ev->file, ev->line, ev->check_msg);
#endif
	vfprintf((FILE*)(ev->udata), ev->fmt, ev->ap);
	fprintf((FILE*)(ev->udata), "\n");
	fflush((FILE*)(ev->udata));
